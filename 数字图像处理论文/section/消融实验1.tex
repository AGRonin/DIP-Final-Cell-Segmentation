...
实际上，本次实验过程为不断累加模块的过程，每个模块的包含与不包含均在第四章有完整的阐述。逆向看整个实验的过程即为消融实验的过程，为此，这里不再做过多的消融实验的阐述，仅进行超参数选取的展示。
...
\bicaption[tab:labelTab传统分水岭算法参数选取结果]{表}{传统分水岭算法参数选取结果}{Table}{The Result of Traditional Watershed Algorithm Parameter Selection}
