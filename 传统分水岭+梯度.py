...
from skimage.morphology import opening, closing, footprint_rectangle, dilation
...
for id_ in tqdm(test_ids):
    ...
    plt.savefig(
        os.path.join(RESULT_DIR, f"{id_}_grad_water.png"),
        dpi=150,
        bbox_inches="tight"
    )
    plt.close()
    ...
    plt.imshow(result)
    plt.axis("off")
    plt.savefig(os.path.join(RESULT_DIR,f"{id_}_grad_water_result.png"),dpi=150,bbox_inches="tight")
    plt.close()
